...
export async function GET() {
  ...
  for (const esp of especialidades) {
    try {
      ...
      if (error) {
        results.push({ nombre: esp.nombre, status: 'error', message: error.message });
      } else {
        results.push({ nombre: esp.nombre, status: 'success', id: data[0]?.id });
      }
    } catch (err) {
      results.push({ nombre: esp.nombre, status: 'error', message: err.message });
    }
  }
